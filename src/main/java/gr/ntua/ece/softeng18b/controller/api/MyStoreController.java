...
@RestController
@RequestMapping("/observatory/api/myshops")
public class MyStoreController {
    ...
    */
    ...
    ResponseEntity<Object> getAllStores(@RequestParam(name="start", required=false) Integer start, @RequestParam(name="count", required=false) Integer count, @RequestParam(name="status", required=false) String status, @RequestParam(name="sort", required=false) String sort, @RequestParam(name="dist", required=false) Double dist, @RequestParam(name="lng", required=false) Double lng, @RequestParam(name="lat", required=false) Double lat) {
        List<Store> stores;
        if(start == null){
            start = 0;
        }
        ...
        try {
            if (dist == null) {
                ...
                if (status.equals("ALL")) {
                    stores = storeRepository.findAllStoresDist(start, count, sort, lat, lng, dist);
                } else {
                    stores = storeRepository.findStoresDist(start, count, sort, status.equals("WITHDRAWN"), lat, lng, dist);
                }
            }
        }
        catch (Exception e) {
    ...
    ResponseEntity<Object> getStore(@RequestParam(name="lat", required=true) Double lat, @RequestParam(name="lng", required=true) Double lng) {
        Optional<Store> showStore = storeRepository.findByCoordinates(lat, lng);
        if (showStore.isPresent()) {
            JSONObject obj = new JSONObject();
            obj.put("id", showStore.get().getId());
    ...
    ResponseEntity<Object> getNewProduct(@RequestBody JSONObject jsonStore, @RequestHeader("X-OBSERVATORY-AUTH") String token) {
        ...
        ResponseEntity<Object> response = new ResponseEntity<>(StoreJsonCreator.storeJson(createdStore), HttpStatus.CREATED);
        return response;
    }
