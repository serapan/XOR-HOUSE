...
//import gr.ntua.ece.softeng18b.security.SecurityConstants;
//import io.jsonwebtoken.Jwts;

@RestController
@RequestMapping("/observatory/api/myprices")
public class MyPricesController {
    ...
    ResponseEntity<Object> getAllProducts(@RequestParam(name="start", required=false) Integer start, @RequestParam(name="count", required=false) Integer count, @RequestParam(name="geoDist", required=false) Integer geoDist, @RequestParam(name="geoLng", required=false) Double geoLng, @RequestParam(name="geoLat", required=false) Double geoLat, @RequestParam(name="dateFrom", required=false) @DateTimeFormat(pattern="yyyy-MM-dd") Date dateFrom, @RequestParam(name="dateTo", required=false) @DateTimeFormat(pattern="yyyy-MM-dd") Date dateTo, @RequestParam(name="shops", required=false) List<String> shops, @RequestParam(name="products", required=false) List<String> products, @RequestParam(name="tags", required=false) List<String> tags, @RequestParam(name="sort", required=false) List<String> sort){
        ...
        String sqlFrom = "FROM product, info, store ";
        ...
        sqlArr[++i] = dateFromStr;
        sqlArr[++i] = dateToStr;
        sqlWhere = sqlWhere + "product.sku = info.sku AND store.store_id = info.store_id AND ( (date_from BETWEEN ? AND ?) OR (date_to BETWEEN ? AND ?) OR (date_from <= ? AND date_to >= ?) ) ";
        if(products!=null){
            String temp = "(";
        ...
        for(Map<String, Object> result : results){
            ...
            List<String> pTagsList = new ArrayList<>(Arrays.asList(temp.split(",")));
            ...
            List<String> sTagsList = new ArrayList<>(Arrays.asList(temp.split(",")));
    ...
    ResponseEntity<Object> insertProduct(@RequestBody JSONObject jsonInfo, HttpServletRequest request, @RequestHeader("X-OBSERVATORY-AUTH") String token){
        ...
        if(!jsonInfo.containsKey("price") || !jsonInfo.containsKey("dateFrom") || !jsonInfo.containsKey("dateTo") || !jsonInfo.containsKey("productId") || !jsonInfo.containsKey("storeId")){
        ...
        try{
            ...
            storeIdStr = jsonInfo.getAsString("storeId");
        }
        catch (Exception e){
        ...
        int howMany = infoRepository.findHowManyLikeThis(productId, storeId, sqlDateFrom, sqlDateTo);
        if(howMany > 0){
            response = new ResponseEntity<>("There is a conflict between dates for that product in that store with another user", HttpStatus.CONFLICT);
            return response;
        }
        try{
            Product infoProduct = entryProduct.get();
            Store infoStore = entryStore.get();
            Info currInfo = new Info(price, infoProduct, infoUser, infoStore, date, dateFrom, dateTo);
            infoRepository.save(currInfo);
            JSONObject obj = new JSONObject();
            obj.put("price", price);
            obj.put("productName", infoProduct.getName());
            obj.put("productId", infoProduct.getId());
            obj.put("productTags", infoProduct.getTags());
            obj.put("shopId", infoStore.getId());
            obj.put("shopName", infoStore.getName());
            obj.put("shopTags", infoStore.getTags());
            obj.put("shopAddress", infoStore.getAddress());
            sqlDateFrom = java.sql.Date.valueOf(dateFromStr);
            sqlDateTo = java.sql.Date.valueOf(dateToStr);
            obj.put("dateFrom", sqlDateFrom);
            obj.put("dateTo", sqlDateTo);
