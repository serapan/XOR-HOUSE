...
@RestController
@RequestMapping("/observatory/api/myproducts")
public class MyProductController {
    ...
    */
    ...
    ResponseEntity<Object> getNewProduct(@RequestBody JSONObject jsonProduct, @RequestHeader("X-OBSERVATORY-AUTH") String token) {
        ...
        ResponseEntity<Object> response = new ResponseEntity<>(JsonCreator.productJson(createdProduct), HttpStatus.CREATED);
        return response;
    }

    @RequestMapping(value = "/findid", method = RequestMethod.POST, consumes = "application/json", produces = "application/json")
    ResponseEntity<Object> getProductId(@RequestBody JSONObject jsonProduct) {
        Product newProduct = JsonCreator.getAProductFromJson(jsonProduct);
        if (newProduct == null) {
            return new ResponseEntity<>("Invalid Product", HttpStatus.BAD_REQUEST);
        }
        JSONObject obj = new JSONObject();
        obj.put("id", JsonCreator.productIsPresentId((ArrayList<Product>) productRepository.findByName(newProduct.getName()), newProduct));
    ...
    ResponseEntity<Object> patchProduct(@RequestBody JSONObject jsonProduct, @PathVariable("productId") Long productId, @RequestHeader("X-OBSERVATORY-AUTH") String token) {
        ...
        Product updatedProduct = JsonCreator.productFromJson(jsonProduct);
        updatedProduct.setRated(existing.getRated());
